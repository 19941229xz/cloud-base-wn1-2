...
public class EurekaListener {
    ...
    public void listen(EurekaInstanceCanceledEvent event){
       ...
       if(appName.equalsIgnoreCase("user")){
           //发邮件通知管理员
           messageFeign.sendTextEmail(TextEmailEntity.setToAndText("3047711845@qq.com","服务下线通知：服务名称-"+appName+
                   " 实例ID:"+event.getServerId()+" 下线时间："+simpleDateFormat.format(new Date(event.getTimestamp()))));
       }
    ...
    public void listen2(EurekaInstanceRegisteredEvent event){
        ...
        System.out.println("服务注册通知：服务名称-"+event.getInstanceInfo().getAppName()+" 实例ID："+event.getInstanceInfo().getId()+" " +
